...
```
浏览器页面                    WebSocket Server              打印服务
xiaoxue-printer-sdk.js  →  127.0.0.1:54321  →  HtmlRenderService
                                ↓                   ↓
                        ┌──────┴──────┐         BartenderManager
                        ↓             ↓              ↓
                   任务管理      任务执行      PrintTaskExecutor
                                        →  系统打印机
```
...
```bash
# 克隆项目
git clone https://gitee.com/iteachyou/xiaoxue-web-printer.git
cd xiaoxue-web-printer

# 编译打包
mvn clean package

# 启动服务
java -jar target/dreamer-print-service-1.0.4.jar
```
...
| type | 参数 | 响应 type | 说明 |
|------|------|----------|------|
| `doPrint` | `printer` / `style` / `content` | `doPrint_result` | 提交并执行打印 |
...
| `getBartenderInstance()` | `Promise<Bartender>` | 获取 BarTender 实例 |
...
脚本（`build/build.ps1`）会自动完成：版本号同步 → ProGuard 混淆 → 复制依赖 → jpackage 生成自带 JRE 的绿色版 → Inno Setup 产出安装包。

产物：`build\windows-晓雪WEB打印控件-<版本>-x86_64.exe`

### 手动打包

> ⚠️ **关键前提**：**不要用 shade 生成的 fat jar 打包**。SWT 的 native 库（dll）打进 fat jar 后会加载失败。

1. **构建**：`Run As → Maven build...`，Goals 填 `clean package`。`target` 下产出：
...
4. **Inno Setup**：`ISCC.exe build/dreamer-print.iss`
...
默认只绑回环地址。若确需局域网访问，改为 `0.0.0.0`，并**自行增加鉴权**。
...
├── pom.xml                              # Maven 构建配置
├── xiaoxue-printer-sdk.js               # 浏览器 SDK 源码
...
├── build/                               # 打包脚本与产物
│   ├── build.ps1 / 打包.bat             # 一键打包脚本
│   ├── dreamer-print.iss                # Inno Setup 脚本
│   ├── dreamer-print.ico                # 应用图标
│   └── proguard.pro                     # ProGuard 混淆规则
├── RELEASE-NOTES.md                     # 版本历史
└── src/main/
    ├── java/cc/iteachyou/printservice/
    │   ├── MainApp.java                 # 启动入口
    │   ├── tray/SystemTrayManager.java  # 托盘管理
    │   ├── websocket/
    │   │   ├── PrintWebSocketServer.java# WebSocket 服务
    │   │   └── PrintTask.java           # 打印任务模型
    │   ├── print/
    │   │   ├── HtmlRenderService.java   # HTML → PDF
    │   │   ├── PrintTaskExecutor.java   # PDF → 打印机
    │   │   ├── PaperSizeUtil.java       # 纸张尺寸工具
    │   │   └── bartender/               # BarTender 集成
    │   ├── ui/                          # UI 对话框
    │   │   ├── AboutDialog.java         # 关于对话框
    │   │   ├── LicenseDialog.java       # 授权对话框
    │   │   ├── PreviewDialog.java       # 打印预览窗口
    │   │   └── TaskListDialog.java      # 任务列表窗口
    │   └── util/                        # 工具类
    │       ├── LicenseManager.java      # 授权管理
    │       ├── MachineCode.java         # 机器码生成
    │       └── VersionUtil.java         # 版本工具
    └── resources/
...
dorkbox 在 Windows 原生托盘上有已知 bug（`bounds is null`）。本项目改用 SWT 原生托盘，彻底规避。
